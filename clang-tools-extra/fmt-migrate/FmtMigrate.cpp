...
public:
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    ...
    auto *D = Result.Nodes.getNodeAs<CompoundStmt>("stream");
    assert(D);
    auto start = D->children().begin();
    if (start->getStmtClass() == clang::Stmt::CXXOperatorCallExprClass) {
      llvm::errs() << "got operatpr call expr class for child "
                   << start->getStmtClassName() << "\n";
    }
  }
...
int main(int argc, const char **argv) {
  ...
  Finder.addMatcher(compoundStmt(isExpansionInMainFile(),
                                 isExpandedFromMacro("ROS_INFO_STREAM"))
                        .bind("stream"),
                    &Callback);
